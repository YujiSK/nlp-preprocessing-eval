...
### 14:34 — 付録見出し階層の統一
- `SUMMARY_REPORT.md`と`SUMMARY_REPORT_extra.md`の付録A・Bを見出しレベル3（`###`）からレベル2（`##`）へ変更し、付録C以降と同じ階層へ統一した。
- 既存の未コミットCSS、PDF、レイアウト検査JSONには触れていない。

### 14:37 — 図A.1〜A.4のキャプション・配置調整開始
- 発展版の図A.1〜A.4について、キャプションの日本語化と2列×2行グリッドへの変更に着手した。
- 再統合時にも維持されるよう、生成元`APPENDIX_EXP_A_PERMUTATION.md`と共通PDF CSSを更新する方針とした。

### 14:40 — 図A.1〜A.4のキャプション・配置調整完了
- 図番号をA.1=k-NN、A.2=ロジスティック回帰、A.3=ランダムフォレスト、A.4=サポートベクターマシン（SVC）の順に揃え、キャプションを日本語化した。
- Markdown-in-HTMLと`.figure-grid-2col`を用い、4図を同一ページの2列×2行へ配置した。各図は既存ビルダーにより`figure/figcaption`へ変換され、レジストリIDも付与された。
- `build_summary.py`で発展版Markdownを再統合し、`build_report.py -t extra`でExtra版のみ再ビルドした。全画像HTTP 200、32ページ、レイアウト違反0件だった。
- レンダリング21ページ目を目視し、4図、全日本語キャプション、後続本文に欠落・見切れ・重なりがないことを確認した。
- 実PDFビルドとは別に非E2Eテスト19件が成功し、`git diff --check`も成功した。

### 14:46 — 発展実験の付録C集約開始
- 独立していた付録C〜Fを、単一の`付録C：発展実験`配下へ実験A〜Dとしてまとめる見出し階層調整に着手した。
- 再統合時にも維持されるよう、`scripts/extra/build_summary.py`の見出し変換処理を更新する方針とした。

### 14:48 — 発展実験の付録C集約完了
- 統合版の階層を`## 付録C：発展実験`、`### 実験A〜D（発展）`、`#### 方法・結果・再現性等`へ変更した。従来の付録D〜F見出しは廃止した。
- `build_summary.py`を更新し、4つの単体Appendixを付録C配下の実験セクションとして再現可能に統合するようにした。
- Extra版を再ビルドし、31ページ、レジストリ109件、レイアウト違反0件、全画像HTTP 200を確認した。
- 付録C・実験A開始ページ、図A.1〜A.4ページ、実験B開始ページを目視し、見出し階層、区切り、本文・図表に欠落や重なりがないことを確認した。
- 見出し階層の機械アサーションと`git diff --check`が成功した。

### 15:24 — 表キャプション・図グリッドCSSの限定化開始
- 表直後の通常本文まで中央揃えになる`table + p, table + p *`を、`em`を含むキャプション段落だけへ限定する調整に着手した。
- `.figure-grid-2col`は生成済みDOMに存在する`figure > img`だけを対象にし、不要な`p`・`.figure-item`セレクタを除去する方針とした。

### 15:27 — 表キャプション・図グリッドCSSの限定化完了
- 表キャプションを`table + p:has(em)`へ限定し、表直後にある通常本文3件には中央揃えが適用されず、キャプション2件だけが対象になることを生成HTMLで確認した。
- 図グリッドを実DOMの`.figure-grid-2col > figure > img`に沿って整理し、画像高を最大170pxに統一した。実験A・Bの各グリッドはいずれも4組の`figure / img / figcaption`を保持している。
- `build_report.py -t extra`で発展版を再ビルドし、28ページ、レジストリ109件、レイアウト違反0件、全画像HTTP 200を確認した。
- 表5.1・表6.1および図A.1〜A.4・図B.1〜B.4の出力ページを目視し、キャプション配置、2列×2行グリッド、後続本文に見切れ・重なりがないことを確認した。`git diff --check`も成功した。

### 15:31 — 主要表6件へのキャプション追加開始
- 本編の表1.1・3.1・4.1、および付録Cの表C.1・C.2・C.3へ、`table + p:has(em)`の対象となる斜体Markdownキャプションを追加する作業に着手した。
- 再統合時に消失しないよう、本編原稿と各発展実験の単体Appendixを生成元として更新する方針とした。

### 15:33 — 主要表6件へのキャプション追加完了
- 本編の表1.1・3.1・4.1、付録Cの表C.1（Permutation Importance）・C.2（閾値最適化）・C.3（D0〜D3 macro-F1要約）へ斜体キャプションを追加し、発展版Markdownを再統合した。
- 生成HTML上で6件すべてが`table + p:has(em)`に一致し、各段落の直前要素が`table`、内部要素が`em`であることを確認した。
- 本編・発展版PDFを再生成し、本編20ページ・発展版32ページ、双方ともレイアウト違反0件、全画像HTTP 200を確認した。
- 表C.1・C.2・C.3の掲載ページを目視し、中央揃え、余白、表・後続要素との分離に見切れや重なりがないことを確認した。`git diff --check`も成功した。

### 15:35 — 付録C全表のキャプション補完・連番整理開始
- 付録C内でキャプションが未設定だったCoverage比較、D0〜D3条件定義、語彙数・前処理コスト、要因別macro-F1差の4表へ斜体キャプションを追加する作業に着手した。
- 付録Cの全7表を文書登場順のC.1〜C.7へ再採番し、各単体Appendixを生成元として更新する方針とした。

### 15:39 — 付録C全表のキャプション補完・連番整理完了
- 付録Cの全7表を文書登場順のC.1〜C.7へ統一し、不足していたCoverage比較、D0〜D3条件定義、語彙数・前処理コスト、要因別macro-F1差の4キャプションを追加した。
- 生成HTMLを全件走査し、本編5/5表、発展版12/12表のすべてが`table + p:has(em)`構造で、付録表番号が`[1, 2, 3, 4, 5, 6, 7]`の順に一度ずつ出現することを確認した。
- 再統合時に実験Bの既存2列図グリッドが失われないよう、単体Appendixにも`.figure-grid-2col`コンテナを反映した。実験A・Bの両グリッドが各4組の`figure > img`を保持することを確認した。
- 本編・発展版PDFを再生成し、本編20ページ、発展版31ページ、レジストリ58件/109件、双方レイアウト違反0件、全画像HTTP 200を確認した。
- 表C.2、C.4〜C.7および実験Bの2列×2行グリッドを目視し、番号、中央揃え、余白、見切れ、重なりに問題がないことを確認した。`git diff --check`も成功した。

### 15:40 — PDF専用ビルド・グリッドキャプション調整開始
- `build_report.py`へ`--pdf-only`/`-p`を追加し、レイアウト検査・自動修復・ページ画像化を省略してHTML/PDF描画だけを行う経路の実装に着手した。
- 実験A・Bの2列グリッド内キャプションを1行表示にする専用CSSと、CLI分岐を保証するテストを追加する方針とした。

### 15:42 — PDF専用ビルド・グリッドキャプション調整完了
- `--pdf-only`/`-p`を実装し、既存Markdown・実験成果物から中間HTMLとPDFだけを更新し、レイアウト検査、自動修復、監査JSON更新、ページ画像化を省略する経路を追加した。`--help`への説明表示も確認した。
- 指定例`build_report.py -t extra --pdf-only`を実行し、HTML/PDFの更新時刻だけが変わり、監査JSONと既存ページ画像の更新時刻が維持されることを確認した。
- 2列グリッド内の`figcaption`/キャプション段落を0.72rem・`white-space: nowrap`へ限定し、実験A・Bの各4キャプションがPDF上で1行表示になることを目視確認した。
- CLI分岐を含むテスト19件が成功した。PDF専用ビルド後に独立検査を実行し、本編20ページ・発展版30ページ、レジストリ58件/109件、双方レイアウト違反0件を確認した。`git diff --check`も成功した。

### 16:05 — 図グリッドの2図単位ページ分割対応開始
- 実験A・Bの4図一括グリッドを、2図1組のグリッド2ブロックへ分割する作業に着手した。
- 各2図ペア内は分割不可を維持し、ペア間では自然な改ページを許可する構造へ変更する方針とした。

### 16:07 — 図グリッドの2図単位ページ分割対応完了
- 実験A・Bの各4図を、2図入り`.figure-grid-2col`×2ブロックへ分割した。各ペアは`break-inside: avoid-page`で保持し、ペア間は通常のブロック境界として改ページ可能にした。
- 生成HTMLを検査し、計4グリッドがそれぞれ正確に2つの`figure`を保持し、A.1/A.2、A.3/A.4、B.1/B.2、B.3/B.4のペアになっていることを確認した。
- Extra版を通常モードで再ビルドし、30ページ、レジストリ109件、レイアウト違反0件、全画像HTTP 200を確認した。
- PDFの21ページ目と23ページ目を目視し、現在の文章量では各4図が同一ページに収まりつつ、2図ペアの配置・1行キャプション・後続本文に見切れや重なりがないことを確認した。ペア間は独立コンテナのため、空き領域が不足する場合のみ2図単位で次ページへ送られる。
- 構造検査時に生成された`outputs/`直下の一時HTMLは削除し、`git diff --check`の成功を確認した。

### 16:28 — レイアウト自動修復パイプラインの安全化開始
- 自動修復の既定無効化、手動・自動設定の分離、短い日本語見出しの照合改善、未解決判定の厳格化、見出しIDの安定化に着手した。
- 通常ビルドでは設定ファイルを一切書き換えず、`--auto-repair`指定時だけ対象レポートの自動生成設定を保存する方針とした。

### 16:41 — レイアウト自動修復パイプラインの安全化完了
- `build_report.py`の既定を非破壊の1回生成・検査へ変更し、`--auto-repair`を明示した場合だけ修復ループを実行するようにした。`--no-repair`も明示指定用の互換オプションとして追加した。
- `layout_overrides.yml`を対象別の`manual.main/extra`と`generated.main/extra`へ分離した。ビルダーにはマージ済み辞書を直接渡し、自動修復時は対象の`generated`だけを一時ファイル・`fsync`・`os.replace`で原子的に保存する実装へ変更した。
- 短い日本語見出しを完全一致とページ内Y座標カーソルで解決し、次ブロックも診断情報として保持するようにした。未解決件数をスコアへ反映し、違反0でも未解決が残る場合は`INDETERMINATE`として成功扱いしないようにした。
- 暗黙見出しのIDを、親見出し階層・意味的slug・本文SHA-1から生成する安定IDへ変更した。兄弟要素の挿入で既存IDが変わらないことをテストした。
- パイプラインからページPNG生成を除外し、終了時に旧HTML・ページPNG・旧レンダリングディレクトリを削除するようにした。正規の中間HTMLとレジストリだけを`outputs/renders/_build/`へ保持する。
- 通常モードで本編・発展版を再生成し、本編20ページ・発展版31ページ、レジストリ58件/109件、双方`PASS`、違反0件・未解決0件を確認した。ビルド前後で設定YAMLのSHA-256が一致し、非破壊経路から保存処理が呼ばれないこともテストで保証した。
- 全28テストが成功した（既知の未標準化Logistic Regression収束警告6件のみ）。`compileall`、CLIヘルプ、`git diff --check`も成功し、旧HTML・ページPNG・設定一時ファイルが残っていないことを確認した。
